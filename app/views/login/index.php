...
<head>
  ...
  <!-- iCheck -->
  <link rel="stylesheet" href="<?= TEMPLATEURL; ?>plugins/iCheck/square/blue.css">
...
<script type="text/javascript">
  $('#myModal').on('show.bs.modal', function (e) {
    var tuser = $(e.relatedTarget).attr('data-user');
    var tpass = $(e.relatedTarget).attr('data-pass');
    var toko = this.value;
        $.ajax({
            url:'<?= BASEURL; ?>playsms/getstatus',
            method:'POST',
            data:{user:tuser,pass:tpass},
            dataType: "JSON",
            success:function(json)
            {
              if(json.datas[i].status.toString()==true){
                
              }else{
                json.datas[i].NO.toString()
              }
            }
        });
        $.ajax({
